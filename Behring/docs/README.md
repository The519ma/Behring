...
The pathology extension preserves that stable case-level contract and adds a nested `pathology_workflow` object for referral, case, processing, test-request, label, report, and image lifecycle data.
...
Pathology-specific intake extensions that should normalize into `pathology_workflow` are documented in `docs/pathology-baserow-field-mapping.md`.

See `docs/adapter-contract.md` for the adapter/core boundary that future OpenELIS work should follow.

See `docs/openelis-mapping-draft.md` for the planning draft that identifies what a future OpenELIS adapter will need to map, without assuming exact endpoints yet.

See `docs/lab-workflow-scope.md` for the higher-level lab scope that this prototype is intended to support beyond the current single-record reporting flow.

See `docs/openelis-role-decision.md` for the current decision boundary between what OpenELIS should own and what the custom middleware should own.

See `docs/pathology-workflow-contract.md` for the staged pathology workflow contract that extends the original flat case model.

See `docs/pathology-baserow-field-mapping.md` for the draft Baserow field expansion needed to support pathology intake and workflow handoff.

See `docs/v0.0.6-ai-verification.md` for the step-by-step runbook to verify the optional OpenAI draft-section branch.

See `docs/current-state.md` for the fastest summary of which baseline is active now and where to resume work.

See `docs/dev-workbench.md` for the local one-click launcher setup for Node-RED, Baserow, OpenELIS, and browser links.
...
The current saved prototype checkpoint is:

- `VERSION` = `0.0.6`

Release notes for this checkpoint are in:

- `docs/releases/v0.0.1.md`

The original release snapshot for `v0.0.1` is:

- `baseline/live-baserow-success/`

The current recommended active baseline is:
...
The pathology workflow extension keeps the same layered design and adds workflow contracts for:

- referral intake
- case handoff to OpenELIS
- gross processing updates
- dynamic stain, IHC, and repeat requests
- label payload generation
- barcode resolution
- case-linked image assets
...
This first version writes back to these prototype fields:
...
- `Workflow snapshot JSON`: long text
...
The Baserow mapping layer matches the exact field labels returned by the live Baserow API. In the current prototype it accepts both the earlier lowercase labels and the title-cased labels returned by the live table.
...
4. Set the real environment variables, restart Node-RED if needed, and test `POST /reporting/process-baserow-row-by-id` with a real `rowId`.
5. Confirm the same Baserow row is updated with `status`, `Workflow snapshot JSON`, `error message`, and `last processed at`.
