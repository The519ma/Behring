...
The pathology extension preserves these fields and adds a nested `pathology_workflow` object for staged pathology data.
...
The pathology workflow extension may deliberately promote some of these, plus new intake fields, into structured nested workflow data. That expansion is defined in `docs/pathology-baserow-field-mapping.md`.
...
- translating normalized workflow results back into Baserow update fields:
  ...
  - legacy fallback: `Draft report` / `draft report`
