...
For this working iteration, the OpenAI draft-section step is optional:

- if `OPENAI_API_KEY` is missing, the flow keeps the deterministic scaffold
- if `OPENAI_API_KEY` is present, the flow can attempt AI draft-section enhancement
- if `OPENAI_MODEL` is missing, the flow defaults to `gpt-4.1-mini`
...
- deterministic non-AI starter text is now added for:
  - `draft_macroscopic_description`
  - `draft_microscopic_description`
  - `draft_impression`
- those starter sections are still draft-only and review-required
- the payload clearly marks that final sign-out is still manual
- an optional AI step can later replace only those three draft sections while keeping the rest of the payload stable

## Optional AI Draft Sections

This working iteration adds an optional OpenAI-backed branch that can improve only these draft-only sections:
...
- if `OPENAI_API_KEY` is not configured, the flow skips the AI call cleanly and preserves the deterministic scaffold
- if `OPENAI_API_KEY` is configured, the flow sends only normalized case data, report handoff structure, and the current draft scaffolds for refinement
- the AI response must match a strict JSON schema with those three fields only
- if the OpenAI request fails or the response is invalid, the scaffold remains in place
- the flow never asks AI for final sign-out language

The final output includes `ai_draft_sections_result` so it is always clear whether:

- AI was skipped
- AI was attempted and applied
- AI was attempted but the scaffold fallback was used
...
- AI is optional and draft-only
- no final-sign-out automation
- no PDF export yet
